...
/**
 * Handles the creation of table `{{%sales_order_store}}`.
 */
class m200829_113453_create_sales_order_store_table extends Migration
{

    /**
     * @type string 表名称
     */
    public $table = '{{%sales_order_store}}';
    ...
    public function up()
    {
        ...
        $this->addPrimarykey('PK_SALES_ORDER_STORE_ID', $this->table, 'id');
        $this->createIndex('IDX_SALES_ORDER_STORE_STATUS', $this->table, 'status');
        $this->createIndex('IDX_SALES_ORDER_STORE_AMOUNT_INCREMENT_ID', $this->table, 'amount_increment_id');
    ...
    {
        $this->setForeignKeyChecks(false);
        $this->dropIndex('IDX_SALES_ORDER_STORE_AMOUNT_INCREMENT_ID', $this->table);
        $this->dropIndex('IDX_SALES_ORDER_STORE_STATUS', $this->table);
        $this->dropPrimaryKey('PK_SALES_ORDER_STORE_ID', $this->table);
