<?php

return [
    /****************** AAAAAAAAAAAAAAA ************************/
    'Admin'  => '管理员',
    'Account' => '账户',
    'Avatar' => '头像',
    'Always' => '总是',
    'Action' => '动作',
    'Article' => '文章',
    'Articles' => '文章',
    'Author'   => '作者',
    'Area'     => '区域',
    'Address'  => '地址',
    'Admin settings' => '管理员设置',
    'Account info' => '账户信息',
    'Author email' => '作者邮箱',
    'Article category' => '文章分类',
    'Article saved' => '文章已保存',
    'Article tag'   => '文章标签',
    'Article title' => '文章标题',
    'Article author' => '文章作者',
    'Article content' => '文章内容',
    'Article tag saved' => '文章标签已保存',
    'Article tag deleted' => '文章标签已删除',
    'Article category saved' => '文章分类已保存',
    'Article category deleted' => '文章分类已删除',
    'Attribute list' => '属性列表',
    'Attribute name' => '属性名',
    'Add category' => '添加分类',
    'Add child category' => '添加子分类',
    'Add role' => '添加角色',
    'Add SKU'   => '添加 SKU',
    'Add brand' => '添加品牌',
    'Add new brand' => '新增品牌',
    'Add new product' => '新增产品',
    'Add tag'   => '添加标签',
    'Address info' => '地址信息',
    'Add new attribute' => '增加属性',
    'Add user: {nickname}' => '添加用户: {nickname}',
    'Add new category' => '添加新分类',
    'Add new product type' => '新增产品类型',
    'Add new article: {title}'  => '增加新文章: {title}',
    'Add article category: {title}' => '增加文章分类: {title}',
    'Add new customer' => '新增客户',
    'Add new article'  => '新增文章',
    'Add new store'    => '新增店铺',
    'Add customer phone' => '添加客户手机号',
    'Add address' => '添加地址',
    'Add supplier' => '增加供货商',
    'Add product'  => '增加产品',
    'Add menu'     => '添加菜单',
    'Add menu action' => '添加菜单操作',
    'Add menu item' => '添加菜单项',
    'Already have account' => '已有账号',
    'Add new customer group' => '新增客户组',
    'Address saved' => '地址已保存',
    'Address list'  => '地址列表',
    'Add new address' => '增加新地址',
    'As default address' => '设为默认地址',
    'Account binded' => '账户已绑定',
    'Allow remember me' => '允许记住我',

    /****************** BBBBBBBBBBBBBBB ************************/
    'Brands'  => '品牌',
    'Brand'   => '品牌',
    'Back'    => '返回',
    'Brand name' => '品牌名称',
    'Brand logo' => '品牌 LOGO',
    'Brand saved' => '品牌已保存',
    'Brand deleted' => '品牌已删除',
    'Brand link' => '品牌链接',
    'Basic info' => '基本信息',
    'Bind other accounts' => '绑定其他账户',
    'Boolean input' => '布尔输入框',
    'Binding email account' => '绑定邮件账户',
    'Binding phone account' => '绑定手机账户',

    /****************** CCCCCCCCCCCCCCC ************************/
    'Create new customer' => '新建客户',
    'Created At'   => '创建时间',
    'Created at'   => '创建时间',
    'Create user'  => '创建用户',
    'Config saved' => '配置已保存',
    'Captcha code' => '验证码',
    'Cost price'   => '成本价',
    'Category'  => '分类',
    'Content'   => '内容',
    'Customers' => '客户',
    'Customer'  => '客户',
    'Catalog'   => '目录',
    'Company'   => '公司',
    'City'      => '城市',
    'Contact'   => '联系人',
    'Copyright' => '版权所有',
    'Company name' => '公司名',
    'Checkbox input' => '多选输入',
    'Checkbox list'  => '多选列表',
    'Checkout quote' => '检出报价',
    'Customer center' => '客户中心',
    'Category info' => '分类信息',
    'Category description' => '分类描述',
    'Content info' => '内容信息',
    'Change password' => '修改密码',
    'Customer saved' => '客户已保存',
    'Customer registered' => '客户注册成功',
    'Customer deleted' => '客户已删除',
    'Customer group'   => '客户组',
    'Customer group name' => '客户组名',
    'Customer group saved' => '客户组已保存',
    'Customer login'  => '客户登录',
    'Category saved' => '分类已保存',
    'Category name'  => '分类名',
    'Customer settings' => '客户设置',
    "Create new category" => "新建分类",
    'Confirm password' => '确认密码',
    'Captcha retry count' => '验证码尝试次数',
    'Current user\'s password' => '当前用户密码',
    'Customer account deleted' => '客户账户已删除',
    'Customer password saved' => '客户密码已保存',
    'Customer address saved'  => '客户地址已保存',
    'Customer logged out'     => '客户已登出',
    'Confirm your registion'  => '注册确认',
    'Cannot delete the default group' => '不能删除默认组',
    /****************** DDDDDDDDDDDDDDD ************************/
    'Disabled'    => '已禁用',
    'Description' => '描述',
    'Delete'      => '删除',
    'Delete at'   => '删除时间',
    'Dropdown list' => '下拉列表',
    'Date of birth' => '生日',
    'Default address' => '默认地址',
    'Default customer group' => '默认客户组',
    'Display this menu' => '显示这个菜单',
    'Display captcha when login' => '登陆时显示验证码',
    'Display captcha retries count' => '显示验证码的登陆失败次数',
    'Display in navbar' => '在导航栏显示',
    'Display in product list' => '在产品列表页显示',
    'Do you want remove this address' => '您要移除这个地址吗',
    'Do you want remove it?' => '是否要移除它? ',
    'Do you want empty shopping cart' => '是否要清空购物车',

    /****************** EEEEEEEEEEEEEEE ************************/
    'Enabled' => '已启用',
    'Email' => '邮件',
    'Edit' => '编辑',
    'Edit customer' => '编辑客户',
    'Edit tag'   => '编辑标签',
    'Edit store' => '编辑店铺',
    'Enable captcha code' => '开启验证码',
    'Enabled by condition' => '有条件的启用',
    'Edit role' => '编辑角色',
    'Edit brand' => '编辑品牌',
    'Edit category' => '编辑分类',
    'Edit attributes' => '编辑属性',
    'Edit attribute' => '编辑属性',
    'Edit product type' => '编辑产品类型',
    'Edit user: {nickname}' => '编辑用户: {nickname}',
    'Edit user: {username}' => '编辑用户: {username}',
    'Edit article category' => '编辑文章分类',
    'Edit article' => '编辑文章',
    'Edit supplier' => '编辑供货商',
    'Edit product'  => '编辑产品',
    'Edit address'  => '编辑地址',
    'Edit menu' => '编辑菜单',
    'Edit menu action' => '编辑菜单操作',
    'Edit menu item' => '编辑菜单项',
    'Edit customer group' => '编辑客户组',
    'Edit article category: {title}' => '编辑文章分类: {title}',
    'Edit user\'s role' => '管理用户角色',
    'Email address' => '邮件地址',
    'Edit customer address' => '编辑客户地址',
    'Edit product type attribute' => '编辑产品类型属性',
    /****************** FFFFFFFFFFFFFFF ************************/
    'Footer' => '尾部',
    'Female' => '女',
    'Frontend' => '前端',
    'Forgot password' => '忘记密码',
    'Faild delete user: {username}' => '删除用户失败: {username}',
    /****************** GGGGGGGGGGGGGGG ************************/
    'Gender' => '性别',
    'General' => '通用',
    'General settings' => '通用设置',
    /****************** HHHHHHHHHHHHHHH ************************/
    'Header' => '头部',
    'Home page' => '首页',
    /****************** IIIIIIIIIIIIIII ************************/
    'Id' => 'ID',
    'Ip' => 'IP',
    'Image' => '图片',
    'Individual' => '个人',
    'Is active'  => '是否激活',
    'Is publish' => '是否发布',
    'Is virtual' => '是否虚拟产品',
    'Is best'    => '是否精品',
    'Is new'     => '是否新品',
    'Is hot'     => '是否热销',
    'Is part'    => '是否配件',
    'Input type' => '输入类型',
    'Index type' => '索引类型',
    'Input url'  => '输入 URL',
    'I agree'    => '我同意',
    'Icp number' => 'ICP 备案号',
    'Icp link'   => 'ICP 官网',
    'Inventory qty' => '库存数量',
    'Is platform store' => '是否为平台店铺',
    'Item is removed' => '项目已经被移除',
    /****************** JJJJJJJJJJJJJJJ ************************/
    'Join time' => '加入时间',
    /****************** KKKKKKKKKKKKKKK ************************/
    'Keywords' => '关键字',
    /****************** LLLLLLLLLLLLLLL ************************/
    'Label' => '标签',
    'Login' => '登录',
    'Logout' => '退出登录',
    'Locked' => '已锁定',
    'Login successful' => '登陆成功',
    'Last login at' => '最近登录时间',
    'Last login ip' => '最近登录 IP',
    'Legal person' => '法人',
    'Login captcha enabled' => '启用登录验证码',
    /****************** MMMMMMMMMMMMMMM ************************/
    'Message' => '消息',
    'Menu'    => '菜单',
    'Male'    => '男',
    'Menu action saved' => '菜单操作已保存',
    'Menu unique code' => '菜单唯一码',
    'Manage user' => '管理用户',
    'Manager log' => '管理员日志',
    'Manage articles' => '管理文章',
    'Manage roles' => '管理角色',
    'Manage users' => '管理用户',
    'Manage menus' => '管理菜单',
    'Meta info'    => '元信息',
    'Meta keywords' => '元关键字',
    'Meta description' => '元描述',
    'Menu action' => '菜单操作',
    'Menu name'   => '菜单名称',
    'Menu code'   => '菜单代码',
    'Menu item saved' => '菜单项已保存',
    'Manage menu actions' => '管理菜单操作',
    'Manage menu items'   =>'管理菜单项',
    'Market price' => '市场价',
    'Manage SKU'    => '管理 SKU',
    'Manage customers' => '管理客户',
    'Manage addresses' => '管理地址',
    'Manage brands'    => '管理品牌',
    'Manage categories' => '管理分类',
    'Manage attributes' => '管理属性',
    'Manage suppliers'  => '管理供货商',
    'Manage stores'     => '管理店铺',
    'Manage products'   => '管理产品',
    'Manage orders'     => '管理订单',
    'Manage deleted products' => '管理已删除的产品',
    'Manage article tags' => '管理文章标签',
    'Manage product types' => '管理产品类型',
    'Manage product type attributes' => '管理产品类型属性',
    'Manage article categories' => '管理文章分类',
    'Mobile number' => '手机号码',
    'Mobile phone number' => '手机号码',
    'My favorite' => '我的收藏',
    'Multiple select dropdown list' => '多选下拉列表',
    'Multiple line text input' => '多行文本输入框',
    /****************** NNNNNNNNNNNNNNN ************************/
    'No'             => '否',
    'Name'           => '名称',
    'Next'           => '下一步',
    'Note'           => '注释',
    'Next step'      => '下一步',
    'No label'       => '无标签',
    'Nickname'       => '昵称',
    'New user added' => '新用户已创建',
    /****************** OOOOOOOOOOOOOOO ************************/
    'Operation' => '操作',
    'On sale'   => '上架',
    'Order'     => '订单',
    'Oauth account' => 'Oauth 账户',
    'Oauth accounts' => 'Oauth 账户',
    'Orders center'  => '订单中心',
    'Original password' => '原始密码',

    /****************** PPPPPPPPPPPPPPP ************************/
    'Password' => '密码',
    'Phone'    => '电话',
    'Price'    => '价格',
    'Product'  => '产品',
    'Permission' => '权限',
    'Published' => '已发布',
    'Params'    => '参数',
    'Pick up'   => '对接人',
    'Price info' => '价格信息',
    'Phone added' => '手机号码已添加',
    'Parent category' => '父分类',
    'Parent menu item' => '父菜单项',
    'Page not found' => '页面未找到',
    'Password info'  => '密码信息',
    'Password changed' => '密码已修改',
    'Product name'   => '产品名',
    'Product sku'    => '产品 SKU',
    'Product price'  => '产品价格',
    'Product type'   => '产品类型',
    'Product weight' => '产品重量',
    'Product type name' => '产品类型名称',
    'Product market price' => '产品市场售价',
    'Product cost price' => '产品成本价格',
    'Product description' => '产品描述',
    'Product restore'     => '产品恢复',
    'Product restored'    => '产品已恢复',
    'Product saved'       => '产品已保存',
    'Product gallery'     => '产品画册',
    'Product category'    => '产品分类',
    'Product stock'       => '产品库存',
    'Promote price'  => '促销价',
    'Product image'  => '产品图',
    'Pending orders' => '未支付订单',
    'Please select' => '请选择...',
    'Product promotion price' => '产品促销价',
    'Product promotion price start date' => '促销起始日期',
    'Product promotion price end date' => '促销结束日期',
    'Product type saved' => '产品类型已保存',
    'Product options'    => '产品选项',
    'Product option'     => '产品选项',
    'Product SKU'        => '产品 SKU',
    'Product sku saved'  => '产品 SKU 已保存',
    'Product sku deleted' => '产品 SKU 已删除',
    'Product shipping rate' => '产品运费',
    'Product virtual sales' => '产品虚拟销量',
    'Product type attribute saved' => '产品类型属性已保存',
    'Please input {attribute}' => '请输入{attribute}',
    'Phone number or email address' => '手机号码或邮件地址',
    'Please input verify code' => '请输入验证码',
    'Please input password' => '请输入密码',
    'Please input confirm password' => '请输入确认密码',
    'Please contact us if you think this is a server error. Thank you.' => '如果您认为这是服务器的错误,请联系我们. 谢谢!',
    'Please remove spaces and newline characters' => '请移除空格和换行符',
    /****************** QQQQQQQQQQQQQQQ ************************/
    'Qq' => 'QQ',
    'Quick to login' => '快速登录',
    /****************** RRRRRRRRRRRRRRR ************************/
    'Role' => '角色',
    'Reset' => '重置',
    'Region' => '省',
    'Restore' => '恢复',
    'Route'   => '路由',
    'Reviewing' => '审核中',
    'Retry count' => '尝试次数',
    'Role info' => '角色信息',
    'Role saved' => '角色已保存',
    'Radio input' => '单选输入',
    'Radio list'  => '单选列表',
    'Register'   => '注册',
    'Review successful' => '审核成功',
    'Review failed'     => '审核失败',
    'Remove address' => '删除地址',
    'Reset password' => '重置密码',
    'Remember me' => '记住我',
    'Remember me time' => '记住我的时间',
    'Resend verify code must be wait {number} second' => '重新发送验证码需要等 {number} 秒',
    /****************** SSSSSSSSSSSSSSS ************************/
    'Sku'  => 'SKU',
    'Save' => '保存',
    'System' => '系统',
    'Submit' => '提交',
    'Submitted' => '已提交',
    'Search' => '搜索',
    'Street' => '街道',
    'Stock'  => '库存',
    'Store'  => '店铺',
    'Sales'  => '销售',
    'Status' => '状态',
    'Supplier' => '供货商',
    'Suppliers' => '供货商',
    'Stock info' => '库存信息',
    'Stock warning' => '库存预警',
    'Status info' => '状态信息',
    'Store name'  => '店铺名',
    'Store type'  => '店铺类型',
    'Store description' => '店铺描述',
    'Show log' => '查看日志',
    'Show list' => '查看列表',
    'Show articles' => '查看文章',
    'Sort order' => '排序顺序',
    'Save only'  => '仅保存',
    'Shop price' => '本店价',
    'System config' => '系统配置',
    'Save and publish' => '保存并发布',
    'Save and continue' => '保存并继续',
    'Show in navbar' => '显示在菜单栏',
    'Switch product type' => '切换产品类型',
    'Supplier deleted' => '供货商已删除',
    'Supplier saved' => '供货商已保存',
    'Shipping rate'  => '运费',
    'Shipping orders' => '运输中订单',
    'Send verify code' => '发送验证码',
    'Shopping cart' => '购物车',
    'Search engine optimize' => '搜索引擎优化',
    'Show warning when stock too low' => '低库存预警',
    'Shopping cart is empty' => '购物车已经被清空',
    /****************** TTTTTTTTTTTTTTT ************************/
    'Time' => '时间',
    'Type' => '类型',
    'Title' => '标题',
    'Tag'   => '标签',
    'Tag name' => '标签名',
    'Text input' => '文本框',
    'Textarea input' => '多行文本框',
    'This is a virtual product' => '这是虚拟产品',
    'This is a part of product' => '这是一个配件',
    'This is a new product' => '这是新品',
    'This is a best sale product' => '这是精品',
    'This is a hot sale product' => '这是热销产品',
    'This product is on sale'    => '将此产品上架',
    'This product has options'   => '这个产品有选项',
    'This user cannot be deleted' => '此用户不能被删除',
    'This {label} already exists' => '这个{label}已经存在',
    'The above error occurred while the Web server was processing your request.' => '服务器在处理您的请求时发生错误',
    /****************** UUUUUUUUUUUUUUU ************************/
    'Url'              => 'URL',
    'User'             => '用户',
    'Username'         => '用户名',
    'Unpublished'      => '未发布',
    'Updated At'       => '更新时间',
    'Updated at'       => '更新时间',
    'Unit "seconds"'   => '单位 “秒”',
    'User saved'       => '用户已保存',
    'User logouted'    => '用户已登出',
    'User was deleted' => '用户已删除',
    'User center'      => '用户中心',
    'User login: {nickname}'  => '用户登录: {nickname}',
    'User logout: {nickname}' => '用户注销: {nickname}',
    'Update article: {title}' => '编辑文章: {title}',
    /****************** VVVVVVVVVVVVVVV ************************/
    'Value'       => '值',
    'Value list'  => '值列表',
    'Verify code' => '验证码',
    'Virtual sales' => '虚拟销量',
    'View {name}' => '查看 {name}',
    /****************** WWWWWWWWWWWWWWW ************************/
    'Website'       => '网站',
    'Weixin'        => '微信',
    'Wechat'        => '微信',
    'Weight'        => '重量',
    'Wishlist'      => '收藏夹',
    'Weight unit'   => '重量单位',
    'Wechat login'  => '微信登录',
    'Website title' => '网站标题',
    'Website description' => '网站描述',
    'Website keywords' => '网站关键字',
    'When login faild' => '当登陆失败时',
    'Write article' => '写文章',
    'Wishlist item deleted' => '收藏的项目已经移除',
    'Welcome to register' => '欢迎注册',
    'When login failed'    => '当登录失败时',
    'Welcome visite {name} shop' => '欢迎访问 {name} 网店',
    /****************** XXXXXXXXXXXXXXX ************************/
    /****************** YYYYYYYYYYYYYYY ************************/
    'Yes' => '是',
    'Your captcha code is {code}' => '您的验证码是 {code}',
    'You must have lastest one Phone or email account' => '至少有一个手机或者邮件账户',
    'You cannot add more than {number} addresses' => '地址不能超过 {number} 条',
    'You cannot remove the default address' => '默认地址不能移除',
    /****************** ZZZZZZZZZZZZZZZ ************************/
    'Zip code' => '邮政编码',
    '{attribute} incorrent' => '{attribute}不正确',
    '{attribute} or {password} incorrent' => '{attribute}或{password}不正确',



];